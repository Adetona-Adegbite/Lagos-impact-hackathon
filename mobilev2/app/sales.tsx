...
export default function ScanSellScreen() {
  ...
  const [mode, setMode] = useState<"sell" | "stock">(
    (params.initialMode as any) || "sell"
  );
  ...
  const onBarcodeScanned = (result: BarcodeScanningResult) => {
    if (isScannerPaused || loading || enterModalVisible || productModalVisible)
      return;
  ...
  if (permission && !permission.granted) {
    return (
      <SafeAreaView className="flex-1 bg-background items-center justify-center p-5">
        ...
        >
          <Text className="font-bold text-lg text-primary-foreground">Grant Permission</Text>
        </Button>
      </SafeAreaView>
    );
  }
