...
export default function CreditProfileScreen() {
  ...
  const exportCSV = async () => {
    try {
      ...
      const rows = SAMPLE_TX.map((t) => [t.id, t.date, `"${t.desc}"`, t.amount, t.type].join(','));
    ...
    try {
      const rowsHtml = SAMPLE_TX.map(
        ...
        <tr>
          ...
          <td style="padding:8px;border:1px solid #eee;text-align:center">${t.type}</td>
        </tr>`
      ).join('');
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.heroWrap}>
          ...
          <Text style={styles.heroSub}>
            Based on 6 months of transaction history, inventory turnover, and cash flow consistency.
          </Text>
        </View>
        ...
        <View style={styles.riskCard}>
          <TouchableOpacity style={styles.riskSummary} activeOpacity={0.9} onPress={() => {}}>
            <View style={styles.riskLeft}>
              <MaterialIcons name="warning" size={20} color="#f59e0b" />
              <View style={{ marginLeft: 10 }}>
                <Text style={styles.riskTitle}>2 Risk Flags Detected</Text>
                <Text style={styles.riskSub}>Review flagged items to keep your score healthy</Text>
              </View>
            </View>
        ...
        <div style={styles.gridHeader}>
          ...
          <TouchableOpacity onPress={() => Alert.alert('Report', 'Open full report')}>
          ...
          <View style={styles.trendBars}>
            {[
              ...
              <View key={i} style={styles.trendBarWrap}>
                ...
                />
                <Text style={[styles.trendLabel, i === 5 ? styles.trendLabelActive : null]}>
                  {b.label}
                </Text>
              </View>
            ))}
        ...
        <View style={styles.narrative}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.narrativeText}>
              Your shop shows strong recurring revenue from staple goods. This profile is optimized
              for micro-loan approval with <Text style={{ fontWeight: '700' }}>92% confidence</Text>
              .
            </Text>
          </View>
        </View>
      ...
      <View style={styles.bottomBar}>
        <TouchableOpacity style={[styles.bottomBtn, styles.pdfBtn]} onPress={exportPDF}>
          <MaterialIcons name="picture-as-pdf" size={18} color="#111" />
          <Text style={styles.bottomBtnText}>PDF</Text>
        </TouchableOpacity>

        <TouchableOpacity style={[styles.bottomBtn, styles.csvBtn]} onPress={exportCSV}>
          <MaterialIcons name="share" size={18} color="#000" />
          <Text style={[styles.bottomBtnText, { color: '#000' }]}>Export CSV</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  riskTitle: { fontWeight: '800', color: '#fff' },
  ...
  riskItemTitle: { fontWeight: '700', color: '#fff' },
  ...
  bottomBtnText: { color: '#000', fontWeight: '800', marginLeft: 6 },
});
